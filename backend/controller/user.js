    ...
    try {
        ...
        if (userEmail) {
            ...
            fs.unlink(filePath, (err) => {
                if (err) {
                    console.log(err)
                    resp.status(500).json({ message: "Error Deleting file" })
                } // else {
        ...
        // console.log("token is send Successfully", activationUrl)
...
// activate-user
router.post("/activation", catchAsyncError(async (req, resp, next) => {
    try {
